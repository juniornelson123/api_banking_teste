defmodule ApiBanking.Financial do
  @moduledoc """
  ...
  def authenticate_user_api(username, plain_text_password) do
  ...
  def list_accounts(user_id) do
    query = from a in Account, where: a.user_id == ^user_id, order_by: [desc: a.inserted_at]
    Repo.all(query)
  end
  ...
  def list_transactions(account_id) do
    query = from t in Transaction, where: t.account_id == ^account_id, order_by: [desc: t.inserted_at]
    Repo.all(query)
  end
  ...
  """
  def list_transfers(transaction_id) do
    query = from t in Transfer, where: t.transaction_id == ^transaction_id, order_by: [desc: t.inserted_at]
    Repo.all(query)
  end
